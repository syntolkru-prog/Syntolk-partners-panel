...
export async function POST(request:NextRequest){
  ...
  const result=await prisma.$transaction(async tx=>{
    ...
    const commission=await tx.commission.upsert({where:{idempotencyKey:`api:earning:${txId}`},update:{},create:{partnerId:referral.partnerId,paymentId:payment.id,kind:"EARNING",idempotencyKey:`api:earning:${txId}`,amount:commissionAmount,rate:referral.partner.commissionRate,availableAt:new Date(Date.now()+settings.commissionHoldDays*86400000)}});
